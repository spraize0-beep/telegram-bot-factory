...
REQUIRED_CHANNELS = ['marketing_azef']
...
async def callback(event):
    ...
    if data == 'back_main':
        ...
        text += f"1️⃣ تقدر تضيف 50 حساب برقم الهاتف\n"
        ...
        await safe_edit(event, "📝 **ابعت الرسالة الرابعه:**\n\nتقدر تبعت نص مع ايموجي بريميوم او ملصق\nالبوت هيحفظه وينشره", buttons=[[Button.inline("🔙 رجوع", b"pub_settings")]])
        return
        ...
        code = gen_code(15)
        await event.answer(f"✅ الكود اتنسخ في الرسالة", alert=True)
        await event.respond(f"🔑 **كود 15 يوم:**\n\n```\n{code}\n```\n\nانسخ الكود وابعت للعميل")
        return
...
async def handle_messages(event):
    ...
    if action == 'code':
        ...
        for user_id in db['users'].keys():
            try:
                await bot.send_message(int(user_id), f"📢 **اعلان من المبرمج**\n\n{msg_text}")
                count += 1
            except:
                pass
        del waiting_for[uid]
        ...
        try:
            ...
            await event.reply("❌ **ارسل رقم صحيح** مثال: 5")
